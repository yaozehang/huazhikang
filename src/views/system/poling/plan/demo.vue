<template>
  <div class="main-content" @click="hideTree($event)">
    ...
    <!-- 新增弹出框 -->
    <el-dialog  class="dialog-form" :title="editType===0?'新增计划':'编辑计划'" width="1000px" :visible.sync="showFlag"
      ...
      <el-row tag="ul">
        ...
        <el-col :span="12" class="input-group">
          <span class="title">执行日期</span>
              <!-- 日期范围 v-if="editParams.templateType ===0"-->
         <el-date-picker v-model="dateList" type="daterange" range-separator="至" start-placeholder="开始日期"
           end-placeholder="结束日期" value-format="yyyy-MM-dd" :disabled="editType===1" @change="changeDate" :picker-options="pickerOptions">
          </el-date-picker>
        </el-col>
        <el-col :span="12" class="input-group">
          <span class="title">巡检频次</span>
...
<!--    <el-col :span="12" class="input-group">
          <span class="title">逾期执行</span>
          <el-switch v-model="isSwitch" active-text="开启" inactive-text="关闭" active-color="#13ce66" inactive-color="#ff4949" @change="setSwitch"> </el-switch>
        </el-col> -->
      </el-row>
          <!-- 部门选择树状图 -->
...
export default {
  ...
  methods: {
   ...
   datedifference(sDate1, sDate2) {    //sDate1和sDate2是2006-12-18格式  
    ...
    regPlan(){
      console.log(this.editParams.userId.length )
      if(this.editParams.templateType !==''&&this.editParams.templateId !==''&& this.dateList.length !==0
       &&this.editParams.userId.length >=2){
        this.regFlag = true
      }else if(this.editParams.templateType ===''){
        this.regFlag = false
         this.$message({ type: 'error', message: '请选择模板类型'})
      }else if(this.editParams.templateId ===''){
        this.regFlag = false
         this.$message({ type: 'error', message: '请选择巡检类型'})
      }else if(this.dateList.length ===0){
        this.regFlag = false
         this.$message({ type: 'error', message: '请选择执行日期'})
      }else if(this.editParams.userId.length ===0){
        this.regFlag = false
         this.$message({ type: 'error', message: '请选择巡检人员'})
      }else if(this.editParams.userId.length ===1){
        this.regFlag = false
         this.$message({ type: 'error', message: '至少选择两名巡检人员'})
      }
    },
    addNewPlan() {
      this.regPlan()
      ...
      let data = {}
      this.editParams.userId = JSON.stringify(this.editParams.userId)
